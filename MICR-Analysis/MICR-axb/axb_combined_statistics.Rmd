...
cont_au_pred_actual <-
  ...
  mutate(predictions =  predict(cont_au_glm, type = "link") )
# cont_au_pred_actual
...
cont_au_pred_actual %>% ggplot(aes(x=step, y=predictions, color=speakerGuise)) %>% 
  ...
  geom_vline(xintercept = bl_x) +
  geom_vline(xintercept = cn_x) +
  geom_vline(xintercept = mi_x) + 
  geom_vline(xintercept = bl_x, linetype="solid") +
  geom_vline(xintercept = cn_x, linetype="dashed") +
  geom_vline(xintercept = mi_x, linetype="dashed")
...
au_data_ind_coef_max <- au_data_coded %>% select(-step:-rt) %>%
  ...
  mutate(xint_trans = xint_pred) %>% #as.vector(scale(xint_pred))) %>%
...
# au_data_ind_coef %>% ggplot(aes(x=xint_trans, y=guise_pred, color=guise_pred)) %>%
# guise_colors(3) +
#   theme_minimal() +
#   coord_cartesian(xlim=c(-3, 3)) +
#   theme(axis.text.y = element_blank()) +
...
xint_distribution <-
au_data_ind_coef %>% ggplot(aes(y=xint_trans, x=speakerGuise, color=speakerGuise, fill=speakerGuise)) %>%
  guise_colors(3) +
...
ind_au_lm_guise <- lm(xint_trans ~ speakerGuise, data=au_data_ind_coef)
summary(ind_au_lm_guise)

au_data_ind_coef$guiseRelvl = relevel(au_data_ind_coef$speakerGuise, ref="CN")
ind_au_lm_guise_relvl <- lm(xint_trans ~ guiseRelvl, data=au_data_ind_coef)
...
au_data_ind_fixef_max <- au_data_coded %>% select(-step:-rt) %>%
  ...
  # Z-score the xint values for comparability + create normalized guise bias/shift effect size score
  mutate(xint_trans = xint) %>% #scale(xint)) %>%
  mutate(guiseShift = ifelse(speakerGuise == 'CN', -xint_trans, xint_trans)) %>%
...
```{r}
# Check outlier removals
setdiff(au_data_ind_fixef, au_data_ind_fixef_noBL) %>% select(-Age:-xint)
```

```{r}
# Check outlier removals
setdiff(au_data_ind_fixef_max, au_data_ind_fixef) %>% select(-Age:-xint)
```
...
xint_distribution <-
au_data_ind_fixef %>% ggplot(aes(y=xint_trans, x=speakerGuise, color=speakerGuise, fill=speakerGuise)) %>%
  guise_colors(3) +
  labs(x='Guise', y='Crossover Point', color='Guise', fill='Guise') +
  gg_theme() +
  theme(legend.position = "none") +
...
ind_au_lm_fixef <- lm(xint_trans ~ speakerGuise, data=au_data_ind_fixef)
summary(ind_au_lm_fixef)

au_data_ind_fixef$guiseRelvl = relevel(au_data_ind_fixef$speakerGuise, ref="CN")
ind_au_lm_fixef <- lm(xint_trans ~ guiseRelvl, data=au_data_ind_fixef)
summary(ind_au_lm_fixef)
```
...
```{r}
# Are the three guises different in xint? YES all differenet
ind_au_lm_guise_bm <- lm(xint_trans ~ speakerGuise, data=au_data_ind_coef_bm_v3)
summary(ind_au_lm_guise_bm)

au_data_ind_coef_bm$guiseRelvl = relevel(au_data_ind_coef_bm$speakerGuise, ref="CN")
ind_au_lm_guise_bm_relvl <- lm(xint_trans ~ guiseRelvl, data=au_data_ind_coef_bm_v3)
