...
save(au_data_coded, file="./data/au_data_coded.rData")
```
...
coef_au <-
  ...
  mutate(xint_bl = -(Intercept / step),
         ...
         slope_bl = step,
         slope_cn = (step + step.speakerGuiseCN),
         slope_mi = (step + step.speakerGuiseMI))

xints_coef_au <-
  coef_au %>% select(participantId, xint_bl, xint_cn, xint_mi) %>%
  pivot_longer(., xint_bl:xint_mi, names_to="guise_pred", names_prefix="xint_", values_to="xint_pred", names_transform = list(guise_pred = toupper)) %>%
  merge(coef_au %>% select(participantId, slope_bl, slope_cn, slope_mi) %>%
          pivot_longer(., slope_bl:slope_mi, names_to="guise_pred", 
                       names_prefix="slope_", values_to="slope_pred", 
...
### Models
```{r}
# ANOVA: Are the three guises different in xint?
ind_au_aov_guise <- aov(xint_pred ~ speakerGuise, data=au_data_ind_coef)
summary(ind_au_aov_guise)

TukeyHSD(ind_au_aov_guise)
```

```{r}
# Basic LM: Are the three guises different in xint? 
...
```{r}
au_data_ind_coef_max %>% filter(slope_out == TRUE | xint_out == TRUE)
au_data_ind_coef_max %>% filter(slope_out == TRUE)
au_data_ind_coef_max %>% filter(xint_out == TRUE)
au_data_ind_coef_max %>% filter(slope_out == TRUE & xint_out == TRUE)
```
...
```{r}
# Check outlier removals
( outs <- setdiff(au_data_ind_coef_max, au_data_ind_coef) %>% select(-Age:-genderContr) )
outs$participantId
...
cont_au_pred_actual %>% filter(participantId %in% flat_slopes_coef) %>%
  ggplot(aes(x=step, y=predictions, color=participantId)) +
  facet_wrap(~speakerGuise) +
  theme_minimal() +
  coord_cartesian(ylim = c(-4, 4)) +
...
# Are the two guises different in guiseShift? 
ind_au_lm <- lm(guiseShift ~ guiseContr,
                data=au_data_ind_coef_noBL)
summary(ind_au_lm)
```
...
save(xints_fixef_au, file="./data/xints_fixef_au.rData")
```
...
```{r}
au_data_ind_fixef_noBL_max %>% filter(slope_pred > quantile(xints_fixef_au$slope_pred, .95))
```

```{r}
au_data_ind_fixef_max %>% filter(slope_out == TRUE | xint_out == TRUE)
au_data_ind_fixef_max %>% filter(slope_out == TRUE)
au_data_ind_fixef_max %>% filter(xint_out == TRUE)
au_data_ind_fixef_max %>% filter(slope_out == TRUE & xint_out == TRUE)
```
...
```{r}
# Check outlier removals
( outs <- setdiff(au_data_ind_fixef_max, au_data_ind_fixef) %>% 
    select(-Age:-guiseRelvl) )
...
```{r}
ind_au_coef <- get_ind_df(xints_coef_au, 0.08)
ind_au_lm <- lm(guiseShift ~ guiseContr + genderContr + scale(Age) + CEscore_all + 
                  SAscore_all * EQscore_all +
                  guiseContr:SAscore_all + guiseContr:EQscore_all + 
                  guiseContr:SAscore_all:EQscore_all, 
                  data=ind_au_coef)
...
ind_au_fixef <- get_ind_df(xints_fixef_au, 0.09)
ind_au_lm <- lm(guiseShift ~ guiseContr + genderContr + scale(Age) + CEscore_all + 
                  SAscore_all * EQscore_all +
                  guiseContr:SAscore_all + guiseContr:EQscore_all + 
                  guiseContr:SAscore_all:EQscore_all, 
