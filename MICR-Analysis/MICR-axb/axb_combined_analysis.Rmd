...
guise_colors <- function(gg_object) {
  gg_object +
...
axb_subj_data %>% ggplot(aes(y=CEscore, x=SAscore, color=speakerGuise, shape=speakerGuise)) %>% guise_colors() %>%
  pca_plot_theme() +
...
byGuise_rt_plot <- condition.means %>% 
  ggplot(aes(x = step, y = grandM.rt)) %>% guise_colors() +
  gg_theme() +
  ...
  mutate(item = as.factor(item),
    ...
    # Deviation/Sum coding (compare to mean of factor)
         ...
         BL.MI=case_when(speakerGuise=='BL' ~ -0.5,
                         speakerGuise=='MI' ~ 0.5,
                         TRUE~0),
         S3.S9=case_when(speaker=='S3' ~ -0.5,
                         speaker=='S9' ~ 0.5,
                          TRUE ~ 0),
    ...
    # Treatment/Baseline coding (compare to reference group)
        ...
        guiseMI=case_when(speakerGuise=='MI' ~ 1,
                         TRUE~0),
        speakerS9=case_when(speaker=='S9' ~ 1,
                         TRUE~0),
        speakerS3=case_when(speaker=='S3' ~ 1,
                         TRUE~0)) #%>%
  
  # Select and reorder relevant columns
  # select(experimentId, participantId:EQscore, respRS, raised_response, rt, rt_sec, 
  #        item, vowel, step, speakerGuise, speaker, word, speakerOrder, conditionId, guiseName, 
  #        step.f, guiseCN, guiseMI, speakerS9, speakerS3, BL.CN, BL.MI, MI.CN, S3.S9)
```

```{r}
# # Select only midpoint data (step 0) for Speaker 3 (sMich)
# axbdata.mid <- axbdata.coded %>% 
#   filter(step == "0") %>% droplevels()  %>% filter(speaker == 'S3') %>%
#   mutate_if(is.character, as.factor)
# 
# # Select only /au/
# AUdata.mid <- axbdata.mid %>% filter(vowel=="AU")  %>% droplevels() 
# 
# # Print to check data
# # AUdata.mid
# # summary(AUdata.mid)
...
## Individual Score Regression (dev)

### Prepare DFs
```{r}

axb_ind_data <- axbdata.coded %>% group_by(participantId, speakerGuise) %>%
  summarize(log_propRS_obs = logit(mean(respRS)), 
            propRS_obs = mean(respRS)) %>% 
  mutate(log_BL.int = fixef(axb.glm.au)[[1]]) %>%
  mutate(propRS_BL.int = inv_logit_scaled(log_BL.int)) %>%
  
  mutate(guiseBias_obs = ifelse(speakerGuise=='MI', -(propRS_obs - propRS_BL.int), (propRS_obs - propRS_BL.int))) %>%
  relocate(guiseBias_obs, .after=speakerGuise)
...
## in Proportions
axb_ind_data %>% ggplot(aes(x=participantId, y=propRS_obs, color=speakerGuise)) %>% 
  guise_colors() +
  gg_theme() +
  geom_point(size=5, alpha=0.7) + 
  geom_abline(aes(slope=0, intercept = propRS_BL.int)) +
  facet_grid(~speakerGuise) +
  labs(y="Observed Proportion 'Raised'", x = "Participant", color = "Guise")


# Boxplot
axb_ind_data %>% ggplot(aes(x=speakerGuise, y=propRS_obs, color=speakerGuise)) %>%
  guise_colors() +
  gg_theme() +
  geom_violin(aes(fill = speakerGuise, color=speakerGuise), alpha=0.3, na.rm=TRUE) +
...
```{r fig.width=10}
axb_ind_data %>% ggplot(aes(x=participantId, y=guiseBias_obs, color=speakerGuise)) %>% 
  guise_colors() +
  gg_theme() +
  geom_point(size=5, alpha=0.7) + 
...
ind.values.prop.long <- ind.values.prop %>%
  pivot_longer(ind.guiseMI:ind.guiseCN, names_to = "speakerGuise", names_prefix = "ind.guise", values_to = "ind.prediction") %>%
  mutate(ind.BLdev = ind.prediction-BL.Intercept) %>%
  select(participantId, listener, speaker, speakerGuise, ind.prediction, BL.Intercept, ind.BLdev)
ind.values.prop.long

```

```{r}
axbdata.ind <- quesdata.final.all %>%
  merge(., ind.values.prop.long)
axbdata.ind

summary(axbdata.ind)

axbdata.mid.au <- filter(filter(axbdata, step==0), vowel=="AU") %>%
  filter(speakerGuise!="BL") %>% droplevels()
axbdata.mid.au
```


```{r}
# Create dataframe of S3 AU midpoint for experimental participants
axbdata.ind.mid.au <- axbdata.ind %>% 
  merge(., axbdata.mid.au, all=TRUE) %>%
  mutate(BL.mean.Prop = case_when(speaker=="S3" ~ 0.4978992,
                                  speaker=="S9" ~ 0.5275424),
         genderContr = case_when(Gender=="f" ~ -0.5,
                                  Gender=="m" ~ 0.5),
         guiseContr = case_when(speakerGuise=="CN" ~ -0.5,
                                speakerGuise=="MI" ~ 0.5),
         speakerContr = case_when(speaker=="S3" ~ -0.5,
                                   speaker=="S9" ~ 0.5),
         speakerS9Contr = case_when(speaker=="S3" ~ 0,
                                   speaker=="S9" ~ 1),
         speakerS3Contr = case_when(speaker=="S9" ~ 0,
                                   speaker=="S3" ~ 1)) %>%
  mutate_if(is.character, as.factor) %>%
  group_by(participantId, listener, Gender, speakerGuise, genderContr, guiseContr, speaker, speakerContr, speakerS9Contr, speakerS3Contr) %>%
  summarise(Age = mean(Age),
            CEscore = mean(CEscore), CSscore = mean(CSscore), MSscore = mean(MSscore), EQscore = mean(EQscore), 
            mean.Pred = mean(ind.prediction), BL.int = mean(BL.Intercept), mean.Prop = mean(respRS), BL.mean.Prop = mean(BL.mean.Prop)) %>%
  mutate(EQgroups = case_when(between(EQscore, 0, 16) ~ "low",
                              between(EQscore, 17, 32) ~ "mid-low",
                              between(EQscore, 33, 48) ~ "mid",
                              between(EQscore, 49, 64) ~ "mid-high",
                              between(EQscore, 65, 80) ~ "high")) %>%
  mutate(EQgroups.HL = case_when(between(EQscore, 0, 43) ~ "low",
                              between(EQscore, 44, 80) ~ "high")) %>%
  mutate(EQgroups.HML = case_when(between(EQscore, 0, 36) ~ "low",
                                 between(EQscore, 37, 49) ~ "mid",
                              between(EQscore, 50, 80) ~ "high")) %>%
  mutate(CSgroups.HML = case_when(between(CSscore, -3, -0.5) ~ "low",
                                 between(CSscore, -0.4999999, 0.4999999) ~ "mid",
                              between(CSscore, 0.5, 3) ~ "high")) %>%
  mutate(BL.diff.Pred = mean.Pred-BL.int, BL.diff.Prop = mean.Prop-BL.mean.Prop,
         guiseBias.Pred = case_when(speakerGuise == "MI" ~ -(BL.diff.Pred),
                                   TRUE ~ BL.diff.Pred),
         guiseBias.Obs = case_when(speakerGuise == "MI" ~ -(BL.diff.Prop),
                                   TRUE ~ BL.diff.Prop))

# Center & scale the EQ variable
#axbdata.ind.mid.au$EQscore.cen <- with(axbdata.ind.mid.au, EQscore-mean(EQscore))
axbdata.ind.mid.au <- data.frame(axbdata.ind.mid.au, EQscore.z=scale(axbdata.ind.mid.au$EQscore))

# Make factors
axbdata.ind.mid.au$EQgroups <- factor(axbdata.ind.mid.au$EQgroups)
axbdata.ind.mid.au$EQgroups.HL <- factor(axbdata.ind.mid.au$EQgroups.HL)
axbdata.ind.mid.au$EQgroups.HML <- factor(axbdata.ind.mid.au$EQgroups.HML, levels = c("high", "mid", "low"))
axbdata.ind.mid.au$CSgroups.HML <- factor(axbdata.ind.mid.au$CSgroups.HML, levels = c("high", "mid", "low"))

axbdata.ind.mid.au <- axbdata.ind.mid.au %>% mutate(EQgroups.4 = as.numeric(cut_number(EQscore, 4))) %>%
  mutate(EQgroups.4 = case_when(EQgroups.4 == 1 ~ "low",
                                EQgroups.4 == 2 ~ "mid-low",
                                EQgroups.4 == 3 ~ "mid-high",
                                EQgroups.4 == 4 ~ "high"))
axbdata.ind.mid.au$EQgroups.4 <- factor(axbdata.ind.mid.au$EQgroups.4, levels = c("high", "mid-high", "mid-low", "low"))

axbdata.ind.mid.au <- axbdata.ind.mid.au %>% mutate(EQgroups.5 = as.numeric(cut_number(EQscore, 5))) %>%
  mutate(EQgroups.5 = case_when(EQgroups.5 == 1 ~ "low",
                                EQgroups.5 == 2 ~ "mid-low",
                                EQgroups.5 == 3 ~ "mid",
                                EQgroups.5 == 4 ~ "mid-high",
                                EQgroups.5 == 5 ~ "high"))
axbdata.ind.mid.au$EQgroups.5 <- factor(axbdata.ind.mid.au$EQgroups.5, levels = c("high", "mid-high","mid", "mid-low", "low"))


# Check data
axbdata.ind.mid.au
summary(axbdata.ind.mid.au)

#write_csv(axbdata.ind.mid.au, "axbdata_ind_au.csv")
