---
title: "jsPsych Preparation: MICRpp"
...
write(sentStimList.json, "./stimLists_MICRpp/sentStimList.json")
```


```{r jsondata-lists-images, include=T}
# Create image list for pre-loading
imageList.json <-  jsonlite::toJSON(imageList, dataframe = "columns")
cat(imageList.json)
write(imageList.json, "./stimLists_MICRpp/imageList.json")
...
# Add in blocks
# Fill column with sequential blockA,B... for the nrow of the DF (evenly distributed) ### 
...
stimulusDF.blocks <- stimulusDF %>%
  mutate(blockName = paste("block",rep(LETTERS[1:3], length.out=nrow(.)),sep="")) %>% 
  group_by(speaker, sentNum) %>% 
  mutate(blockName = sample(blockName)) %>%
  ungroup() %>%
  print()
...
blockList <- paste0("block",rep(LETTERS[1:3]))
...
for (block in blockList){
  ...
  stimuli.json <- jsonlite::toJSON(stimulusDF.exp)
    #cat(stimuli.json)
    write(stimuli.json, paste0("./stimLists_MICRpp/stimuli_",block,".json"))
}
