...
expName = 'MICR_reading'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\laure\\Documents\\U-M\\Research\\QRP\\Methods\\Experiment\\MICR_reading\\MICR_reading.py',
...
text_2 = visual.TextStim(win=win, name='text_2',
    text='In this part of the experiment, you will be asked read words out loud. Your voice will be recorded during this task.\n\nOne word will be shown at a time. Read the word once, then press the right arrow key to see the next word.\n\nWhen you are ready to start, press space bar to continue.',
    font='Arial',
...
for thisBlock in blocks:
    ...
    # set up handler to look after randomisation of conditions etc
    trials_56 = data.TrialHandler(nReps=1, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('wordStim.csv'),
        seed=None, name='trials_56')
    thisExp.addLoop(trials_56)  # add the loop to the experiment
    thisTrial_56 = trials_56.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisTrial_56.rgb)
    if thisTrial_56 != None:
        for paramName in thisTrial_56:
            exec('{} = thisTrial_56[paramName]'.format(paramName))
    
    for thisTrial_56 in trials_56:
        currentLoop = trials_56
        # abbreviate parameter names if possible (e.g. rgb = thisTrial_56.rgb)
        if thisTrial_56 != None:
            for paramName in thisTrial_56:
                exec('{} = thisTrial_56[paramName]'.format(paramName))
        ...
        key_resp_5.rt = []
        # keep track of which components have finished
        trialComponents = [text_5, key_resp_5]
        ...
        for thisComponent in trialComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        trials_56.addData('text_5.started', text_5.tStartRefresh)
        trials_56.addData('text_5.stopped', text_5.tStopRefresh)
        # check responses
        if key_resp_5.keys in ['', [], None]:  # No response was made
            key_resp_5.keys = None
        trials_56.addData('key_resp_5.keys',key_resp_5.keys)
        if key_resp_5.keys != None:  # we had a response
            trials_56.addData('key_resp_5.rt', key_resp_5.rt)
        trials_56.addData('key_resp_5.started', key_resp_5.tStartRefresh)
        trials_56.addData('key_resp_5.stopped', key_resp_5.tStopRefresh)
        #if trials_56.thisN == 56:
        #    if blocks.thisN == 3:
        #        goToEnd = 1
    ...
    # completed 1 repeats of 'trials_56'
