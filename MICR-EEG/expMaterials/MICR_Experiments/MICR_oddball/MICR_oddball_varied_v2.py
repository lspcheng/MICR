...
sound_1 = sound.Sound('A', secs=0.6, stereo=True, hamming=True,
    name='sound_1')
sound_1.setVolume(1)
...
for thisBlock_4 in blocks_4:
    ...
    for thisTrial_500 in trials_500:
        ...
        Trigger= shuffled_list[trials_500.thisN][4]
        sound_1.setSound(shuffled_list[trials_500.thisN][3], secs=0.6, hamming=True)
        sound_1.setVolume(1, log=False)
        # keep track of which components have finished
        trialComponents = [sound_1, image_2]
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if sound_1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > sound_1.tStartRefresh + 0.6-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if image_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > image_2.tStartRefresh + 0.6-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        trials_500.addData('image_2.started', image_2.tStartRefresh)
        trials_500.addData('image_2.stopped', image_2.tStopRefresh)
        thisExp.nextEntry()
